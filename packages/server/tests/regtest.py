...
class GameStateRemGlk(GameState):
    ...
    def read_one_json(self):
        """Read a single complete JSON object from the interpreter's stdout.
        ...
        while (select.select([self.outfile],[],[],opts.timeout_secs)[0] != []):
            ch = self.outfile.read(1)
            if ch == b'':
                # End of stream. Try to parse what we have.
                if output:
                    dat = output.decode('utf-8').strip()
                    if dat:
                        try:
    ...
    def accept_output(self):
        # Read JSON objects until we get one that signals input readiness
        # (has 'input' or 'specialinput' fields, or no 'disable' field).
        # This handles interpreters that send multiple streaming updates
        # per turn.

        timeout_time = time.time() + opts.timeout_secs

        # Reset story window at start of turn so streaming updates accumulate
        self.storywin_reset_pending = True

        while True:
            if time.time() >= timeout_time:
                raise Exception('Timed out awaiting output')

            update = self.read_one_json()
            if update is None:
                # EOF or timeout - no more output
                break

            self.parse_remglk_update(update)

            # If the game has exited, we're done
            if update.get('exit'):
                break
            # If this update signals input readiness, we're done
            if update.get('input') is not None or update.get('specialinput') is not None:
                break
            # If the update doesn't have 'disable', it's also a final update
            if not update.get('disable'):
                break
        ...
        if contents is not None:
            for content in contents:
                ...
                if win.get('type') == 'buffer':
                    if getattr(self, 'storywin_reset_pending', False):
                        self.storywin = []
                        self.storywindat = []
                        self.storywin_reset_pending = False
